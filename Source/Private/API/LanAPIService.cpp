...
void LanApiService::GetProxies(std::function<void(std::optional<std::vector<KyberProxy>>)> callback)
{
        KYBER_LOG(LogLevel::Info, "Returning empty proxy list");
        //callback(std::nullopt);
        callback(std::vector<KyberProxy>());
}

int LanApiService::GetPing(std::string ip)
{
...
void LanApiService::GetServerList(int page, std::function<void(int, std::optional<std::vector<ServerModel>>)> callback)
{
    KYBER_LOG(LogLevel::Error, "Returning empty server list");
